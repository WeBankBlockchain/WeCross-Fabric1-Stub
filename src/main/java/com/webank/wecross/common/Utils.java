...
public class Utils {
     ...
     * @throws IOException
     */
    private static byte[] generateTarGzInputStreamBytes(File src, String pathPrefix) throws IOException {
        pathPrefix = "src/" + pathPrefix; // Huuugggeee hole here!!! the path in tar.gz must start with src/ and then with pathPrefix (sad)
    ...
    private static InputStream generateTarGzInputStream(File src, String pathPrefix) throws IOException {
    ...
    public static byte[] generateTarGzInputStreamBytes(File src) throws IOException {
        return generateTarGzInputStreamBytes(src, "chaincode"); // Inside tar.gz is: src/chaincode/<where chaincode_main.go is>
    }

    public static InputStream generateTarGzInputStream(File src) throws IOException {
        return generateTarGzInputStream(src, "chaincode"); // Inside tar.gz is: src/chaincode/<where chaincode_main.go is>
    }
}
